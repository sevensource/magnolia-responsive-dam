...
public class AspectAwareDamLinkField extends CustomField<String> {
	...
	private transient List<DamVariationSet> variationSets;
	...
	public AspectAwareDamLinkField(LinkField linkField, FormDialogPresenterFactory formDialogPresenterFactory,
			DialogDefinitionRegistry dialogDefinitionRegistry, UiContext uiContext, SimpleTranslator i18n) {
	...
	private void onValueChange() {
		...
		final AspectAwareDamLinkFieldValidator validator = (AspectAwareDamLinkFieldValidator) getValidators().stream()
		...
		if (validator != null) {
			if (!validator.isImage(getValue())) {
				...
				AspectAwareUiUtils.updateInfoLabel(this.infoLabel, i18n.translate(aspectsIncompleteErrorTxt),
						InfoLabelStyle.ERROR);
			}
		}
	}
	...
	private void openEditor() {
		...
		try {
			DefinitionProvider<DialogDefinition> dialogDefinitionProvider = dialogDefinitionRegistry
					.getProvider(getAspectsAppName());
		...
		final FormDialogPresenter formDialogPresenter = formDialogPresenterFactory
				.createFormDialogPresenter(getAspectsAppName());
		...
		for (TabDefinition tabDefinition : dialogDefinition.getForm().getTabs()) {
			for (FieldDefinition fieldDefinition : tabDefinition.getFields()) {
				if (fieldDefinition instanceof AspectAwareDamUploadFieldDefinition) {
					final List<String> variationSetNames = variationSets.stream().map(DamVariationSet::getName)
							.collect(Collectors.toList());
					((AspectAwareDamUploadFieldDefinition) fieldDefinition).setVariationSets(variationSetNames);
				}
			}
		}
	...
	public void setVariationSets(List<DamVariationSet> variationSets) {
